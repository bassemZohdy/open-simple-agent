...
| State/durability | OSA `SessionProvider` bridged into ADK sessions; A2A task state is process-local by default with an optional PostgreSQL SDK store, scoped OSA ownership leases/fencing, atomic fenced task saves, and fail-closed owner loss, while the SDK active-task registry and full replica recovery remain open | OSA `SessionProvider` remains authoritative; optional LangGraph checkpointer is experimental and has no validated durable production contract |
