...
- With a DSN, agents/deployments/resources/audit events/external-agent records
  ...
  durable Control Plane requires `OSA_DEPLOY_PROVIDER=kubernetes` or
  `OSA_DEPLOY_PROVIDER=openshift`; the local provider is development-only and
  process-local.
...
- Set `OSA_PERSISTENCE_POLICY=shared` for a fail-closed production posture:
  ...
  PostgreSQL; the Control Plane requires PostgreSQL plus Kubernetes or
  OpenShift.
...
| `OSA_OPENSHIFT_IMAGE` | Runtime image used by the OpenShift provider | required for `openshift` |
| `OSA_OPENSHIFT_NAMESPACE` | OpenShift project/namespace | `default` |
| `OSA_OPENSHIFT_REPLICAS` | Desired OpenShift runtime replicas | `1` |
| `OSA_OC` | `oc` executable | `oc` |
| `OSA_OPENSHIFT_ROLLOUT_TIMEOUT_SECONDS` | Maximum OpenShift rollout wait | `60` |
| `OSA_OPENSHIFT_ROUTE_HOST` | Optional explicit OpenShift Route host | generated by OpenShift |
| `OSA_OPENSHIFT_ROUTE_TLS_TERMINATION` | OpenShift Route TLS termination (`edge`, `reencrypt`, `passthrough`, or empty) | `edge` |
| `OSA_OPENSHIFT_ROUTE_INSECURE_EDGE_POLICY` | OpenShift Route insecure edge policy (`Allow`, `None`, or `Redirect`) | `Redirect` |
...
see the [OpenShift guide](openshift.md) for the cluster acceptance boundary.
