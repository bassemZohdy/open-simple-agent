...
## Architecture Notes
- uv workspace with four members (`generic-agent`, `runtimes/adk`, `runtimes/langgraph`, `control-plane/backend`) sharing one PEP 420 namespace package `osa`. Never add `__init__.py` at namespace levels (`src/osa/`, `src/osa/runtimes/`, `src/osa/control_plane/`) — it breaks cross-member imports.
...
- Package versions are one lockstep Python release (root + four Python manifests), enforced by `tests/unit/test_versioning.py`; bump all five together and refresh `uv.lock`. The private Control Panel package is not a separately published Python artifact.
