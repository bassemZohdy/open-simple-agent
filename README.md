...
> The remaining production-readiness work is tracked in `TODO.md`: the
> remaining public distributed A2A streaming-route/late-event contract, deployment-specific
> browser OIDC, and release decisions.
> Durable sessions, migration-owned memory schema, replica-wide capability
> telemetry, and the opt-in HTTP rate-limit contract
...
| MCP | Runtime client (stdio + Streamable HTTP + explicitly selected legacy SSE), lazy pooled connections, filtered namespaced tools bridged to ADK, application-controlled resource/prompt discovery and retrieval, bounded results, API-key/OAuth2/mTLS outbound credentials; dual-major MCP 1.x/2.x compatibility coverage | LangGraph MCP integration remains deferred |
...
| ADK runtime | Invocation through the ADK `Runner`; timeouts, iteration limits, stable error types; SSE streaming (`/v1/invoke/stream`) with stable OSA events and disconnect cancellation; A2A Agent Card + JSON-RPC server (ADR-005); optional SQLAlchemy task store with scoped ownership leases, atomic fenced task saves, fail-closed owner loss, independent-handler lookup/cancellation, expired-owner cancellation takeover, terminal read-only replay, terminal-event drain-before-release, a migration-owned schema-version-2 event cursor and bounded polling-relay foundation, and process-boundary plus independent-worker PostgreSQL acceptance via `OSA_A2A_TASK_DATABASE_URL` | PostgreSQL is required for shared task state; run `osa-a2a-migrate` before startup; SQLite is local-only; durable relay takeover/late-event/cursor acceptance is covered, while public cross-process route integration remains open; expired-owner retries fail closed and never replay unknown side effects |
...
| Release | Lockstep release validation; four Python distributions; GHCR and Docker Hub runtime/Control Plane images; SBOM/provenance attestations; keyless Cosign image signing; GitHub Releases with checksums; immutable-digest channel rollback | Current public release `v0.1.3` is complete; optional Python package-registry publication remains open |
...
- [OpenShift guide](docs/guides/openshift.md) — dedicated provider configuration and acceptance requirements
...
intentional version/tag. The current public release is `v0.1.3`; the remaining
