...
| ADK runtime | Invocation through the ADK `Runner`; timeouts, iteration limits, stable error types; SSE streaming (`/v1/invoke/stream`) with stable OSA events and disconnect cancellation; A2A Agent Card + JSON-RPC server (ADR-005); optional SQLAlchemy task store with scoped ownership leases, atomic fenced task saves, fail-closed owner loss, independent-handler lookup/cancellation, expired-owner cancellation takeover, terminal read-only replay, and process-boundary PostgreSQL acceptance via `OSA_A2A_TASK_DATABASE_URL` | PostgreSQL is required for shared task state; run `osa-a2a-migrate` before startup; SQLite is local-only; multi-process streaming/late-event acceptance and non-idempotent replay policy remain open |
