...
| MCP | Runtime client (stdio + Streamable HTTP), lazy pooled connections, filtered namespaced tools bridged to ADK, bounded results, API-key/OAuth2/mTLS outbound credentials | The runtime is pinned to MCP SDK 1.x while the MCP 2.x client/fixture port and ADK validation are in progress; resources/prompts exposure and legacy SSE remain deferred |
